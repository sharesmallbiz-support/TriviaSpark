...
const SimpleProgress = ({ value, className }: { value: number; className?: string }) => {
  ...
  const widthClass = 
    progressWidth >= 90 ? 'w-full' :
    progressWidth >= 80 ? 'w-4/5' :
    progressWidth >= 75 ? 'w-3/4' :
    progressWidth >= 66 ? 'w-2/3' :
    progressWidth >= 60 ? 'w-3/5' :
    progressWidth >= 50 ? 'w-1/2' :
    progressWidth >= 40 ? 'w-2/5' :
    progressWidth >= 33 ? 'w-1/3' :
    progressWidth >= 25 ? 'w-1/4' :
    progressWidth >= 20 ? 'w-1/5' :
    progressWidth >= 10 ? 'w-1/12' :
    'w-0';
  
  return (
    <div className={`w-full bg-gray-200 rounded-full h-2 ${className} relative overflow-hidden`}>
      <div className={`bg-champagne-400 h-2 rounded-full transition-all duration-300 ${widthClass}`} />
    </div>
  );
};
...
export default function StaticPresenterDemo() {
          ...
          <div className="w-full max-w-7xl h-full flex flex-col">
            <Card className="bg-white/10 backdrop-blur-sm border-white/20 text-white flex-1 flex flex-col relative overflow-hidden">
              {currentQuestion.backgroundImageUrl && (
                <div className="absolute inset-0 bg-cover bg-center bg-wine-900">
